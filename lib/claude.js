...
### 3. 경과기록(SOAP) 및 메모 활용
- 환자 데이터에 progressNotes(경과기록)가 있으면 환자 상태, 주소증, 치료 경과를 파악하는 데 참고하세요.
- memo(환자 메모)가 있으면 주소증, 이전 병력, 특이사항 등을 파악하는 데 참고하세요.
- 경과기록이나 메모에서 이전 치료 병원, 수술 이력, 항암 치료 이력 등이 확인되면 【】 없이 직접 기재하세요.
