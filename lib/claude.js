...
const TREATMENT_DESCRIPTIONS = {
  ...
  '셀레늄|셀레나제|세파셀렌|selenium|셀렌': {
    title: '셀레늄(Selenium) 제제: 셀레나제, 세파셀렌 등',
    desc: '셀레늄은 25가지가넘는 효소의 구성요소로서 항산화, 항염증, 갑상선호르몬 안정화작용을 하는 필수미량원소로 유리 라디칼 제거, 면역자극, 항종양효과(p53 활성화를 통한 암세포 자연사 유도, 신생혈관 억제, CDB 활성화, NF-kappaB 활성화 억제, 암세포내 셀레노디글루타치온 합성 증가로 유리 라디칼 증가에 따른 세포 독성효과)를 가지고 있어 암치료에 효과적으로 사용되고 있음. 또한 유방암 또는 기타 다른 종양의 임파선 전이로 인한 부종에 효능이 있음.',
  },
...
### 3. 경과기록(SOAP) 및 업무메모 활용
- 환자 데이터에 progressNotes(경과기록/SOAP)가 있으면 환자 상태, 주소증, 치료 경과를 파악하는 데 참고하세요.
- workMemos(업무메모)가 있으면 업무 관련 특이사항, 치료 세부사항 등을 파악하는 데 참고하세요.
- 경과기록이나 업무메모에서 이전 치료 병원, 수술 이력, 항암 치료 이력 등이 확인되면 【】 없이 직접 기재하세요.
